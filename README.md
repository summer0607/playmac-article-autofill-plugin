...
- 图片：插件自己的后台任务下载、上传并验证千帆外链，文章只写入 `qimg.xiaohongshu.com` 图片。
- 千帆：在插件设置页初始化组件并登录一次；账号密码仅用于当次登录，不写入数据库或插件文件。登录会话保存在网站临时目录的私有文件中。
- Macked：插件直接读取并解析；无法可靠读取时不会保存草稿，也不会使用旧缓存替代。
- 安全：插件不自动发布，不覆盖已有价格和下载链接。

安装后：

1. 打开“设置 → PlayMac 文章自动补全”，点击“初始化插件组件”。
2. 在同一页面登录千帆图片空间。
3. 新建文章，粘贴 Steam 或 Macked 链接并保存草稿。
...
2. 提交并推送代码。
3. 推送同名标签，例如 `v2.0.1`。
4. GitHub 会自动发布 `playmac-article-importer.zip`，供网站更新下载。
